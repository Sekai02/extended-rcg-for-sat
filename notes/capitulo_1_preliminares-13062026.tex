...
usados en el resto del trabajo. \quitaesto{Primero,} en la
sección~\ref{sec:teoria-lenguajes}, se introducen las nociones
básicas de la teoría de lenguajes\quitaesto{: alfabetos, cadenas, lenguajes,
  gramáticas y la jerarquía de Chomsky}\comment{}{la razón por la que te digo que quites esto es porque ya lo pones abajo en la introducción de la sección}. Después, en la
sección~\ref{sec:automatas}, se \comment{presenta el autómata finito}{así escrito parecería que solo hay UN autómata finito XD, mejor ponlo en plural}. Por último,
...
restricciones en \queesesto{las reglas de producción de las gramáticas que los
  generan}.
...
$\Sigma = \{0, 1\}$ está formado por los símbolos $0$ y $1$; \comment{$11$ y $101$
son cadenas sobre el alfabeto $\Sigma$}{esto mejor escribelo al revés `algunas cadenas sobre este alfabeto son fulano y fulano' para que no tengas los 1s pegados ahí}, y un \quitaesto{ejemplo de} lenguaje \agregaesto{sobre este alfabeto} es el
conjunto de cadenas formadas por $0$ y $1$ que terminan en $1$.

\comment{El símbolo $\varepsilon$ representa la \textbf{cadena vacía}. La
\textbf{longitud} de una cadena $w$, denotada $|w|$, es la cantidad de
símbolos que la componen; en particular, $|\varepsilon| = 0$}{creo que deberías hablar de la longitud primero, o decir que la cadena vacía no tiene símbolos. De lo contrario el lector se entera de lo que es la cadena vacía una vez llega al final del párrafo}.

Dadas dos cadenas $w_1$ y $w_2$, la \textbf{concatenación} $w_1 w_2$
se obtiene al \comment{escribir}{no me gusta esta palabra, escribir es una operación o algo así en teoría de lenguajes?} los símbolos de $w_1$ seguidos de los símbolos
...
\comment{Por ejemplo, dado el lenguaje $L$ de las cadenas binarias que terminan
en $0$, el problema de la palabra para la cadena $1100100$ consiste en
determinar si $1100100 \in L$.}{aquí me parece que esto no está bien, el problema de la palabra es un problema para un lenguaje dado, o sea, `dada una cadena \textbf{arbitraria} w y un lenguaje L, determinar si w pertenece a L' no esta bien que digas `dada esta cadena específica w y un lenguaje L, determinar si w pertenece a L', eso no es un ejemplo del problema de la palabra (me parece)}
...
decidir si tal lenguaje tiene algún elemento. \agregaesto{NUEVO PARRAFO}
...
\quitaesto{Una vez introducidos los conceptos básicos y los dos problemas
...
\begin{definicion}
  ...
  \begin{itemize}
    ...
    \item $P$ es un conjunto finito de \textbf{reglas de producción} de la
          ...
          \]}{me parece que $\alpha$ no es terminal, o sea, $\alpha \in (N \cup \Sigma)^* - \Sigma^*$}
...
\comment{$\alpha \to \beta$, donde $\alpha, \beta \in (N \cup \Sigma)^*$}{lo mimso con respecto a $\alpha$ aquíx} y
$\alpha \neq \varepsilon$. Todo lenguaje generado por una gramática
irrestricta se denomina \textbf{lenguaje recursivamente
  enumerable}~\cite{hopcroft2006introduction}.
...
se puede ilustrar con una \comment{familia}{por que se utiliza de forma diferente el término? las familias de lenguajes son cojuntos de lenguajes, porque los lenguajes generados por gramáticas son clases y no familias también?} \footnote{El término "familia de
...
emplea este lenguaje para ilustrar que las gramáticas de concatenación
de rango son más expresivas que las gramáticas libres del contexto.
...
\comment{Los autómatas correspondientes a las cuatro clases de la jerarquía de
...
lenguajes libres del contexto}{por que mencionas a toda esta gente y luego solo defines los autómatas finitos?} y el autómata finito para los lenguajes
regulares~\cite{hopcroft2006introduction}.
...
\comment{Esta sección}{una de las cosas que me señaló el oponente del documento, esra que presentaba de forma informal las cosas de complejidad. Te sugiero que definas formalmente lo que es un problema, una instancia, un problema de decisión, un certifiado, verificador polinomial, etc. para que no te pase lo mismo} se organiza en tres partes. Primero se presenta la
notación asintótica\quitaesto{.}\agregaesto{COMA} Después se introduce la reducción polinomial.
...
o \comment{no}{lo de la nota al pie, no me queda claro, yo mejor quitaría la nota al pie, al final no te hace falta hablar de todos los problemas, solo los de decisión. La nota al pie crea más ruido del que quita}.\footnote{Todo problema computacional admite una reformulación
...
constituye uno de los problemas abiertos \comadreja{centrales} de la teoría de la
...
polinomial, en una fórmula en CNF que es \comment{satisfacible}{que es esto? no lo has definido todavía. Te sugiero que cambies el orden de las cosas} si y solo si la
...
Una fórmula booleana \comment{en CNF}{para las fórmulas arbitrarias no existe este concepto?} es \textbf{satisfacible} si existe una
...
del problema $n$-SAT se puede reducir a una fórmula \comment{equivalente}{quivalente o equisatisfacible?} del
...
Las variantes anteriores se obtienen al restringir la forma de las
cláusulas. Otra forma de ampliar las subclases del SAT con solución en
tiempo polinomial consiste en modelar el problema mediante formalismos
de la teoría de lenguajes. En este capítulo se introdujeron las nociones
en que se apoya ese enfoque: la teoría de lenguajes, \comment{el autómata finito}{solo un autómata finito?} y,
la complejidad computacional y el problema SAT. En el próximo capítulo
se presentan las gramáticas de concatenación de rango y los antecedentes
en el estudio del SAT con teoría de lenguajes.
