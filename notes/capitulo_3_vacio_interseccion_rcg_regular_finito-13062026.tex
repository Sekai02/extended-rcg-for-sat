...
\begin{definicion}
  Un \textbf{lenguaje regular finito} \comment{es un lenguaje finito que también
  es regular.}{esto tienes que escribirlo al reves `un lenguaje regular que tambien es finito', de lo contrario es redundante. Todos los lenguajes finitos son regulares}
\end{definicion}
...
\comment{El pseudocódigo de $\proc{Useful-Part}$ es el siguiente.}{si crees que la tesis se te hace muy larga, yo quitaría el useful part ese, y diría `sin pérdida de generalidad, se asume que el autómata no tiene estados inútiles ya que en tiempo polinomial se puede obtener un autómata equivalente sin estados inútiles [libro de autómatas]'.}
...
\comment{}{estaría bueno que pusieras los algoritmos en español, o al menos que los comentarios estén en español.}
...
\begin{lema}
  ...
  \comment{Toda cadena $w$ para la que el algoritmo ejecuta
  $\proc{RCG-Recognize}(G,w)$ pertenece a $L(A)$}{pon, dado un autómata $A$ (sugiero además que pongas una $\mathcal{A}$ fancy, para que no se confunda con un no terminal) y una gramática $G$ se cumple que ..., si no $G$ y $A$ son variables no definidas. Además pon que $w$ es reconocida por el autómata $A$. O sea, pon cada supuesto del lema}
\end{lema}
...
\begin{lema}
  ...
  \comment{\const{true}}{sería bueno revisar que seas consistente con los TRUE, verdadero y 1 en todo el trabajo} si y solo si $w \in L(G)$.
\end{lema}
...
acíclicos. \comment{En consecuencia, el algoritmo del presente
capítulo no contradice la NP-completitud del caso finito con autómatas
acíclicos demostrada en~\cite{bertsch2001rcg} }{incluso si tu encontraras un algoritmo polinomial, tampoco contradices la NP-completitud del problema, eso está mal dicho. Por ejemplo, SAT es NP-completo haya o no un algoritmo polinomial que lo resuelva}. La enumeración
...
\comment{tratabilidad}{es tratabilidad o tractabilidad?} para clases restringidas de autómatas acíclicos.

\begin{teorema}
  \label{teo:clases-pol-caminos}
  Sea \comment{$\mathcal{A}$}{oh, veo que usas ya la A fancy para otra cosa} una clase de autómatas finitos acíclicos tal que
...
\begin{proof}
  ...
  \[
    O\bigl( |A| + p(A)(|Q| + R_G(|Q|-1)) \bigr)
  \]
  \comment{}{ahora me perdí, que es $|A|$? no era lo mismo que $|Q|$?}
...
\begin{proof}
  ...
  \comment{$w_I = b_1 b_2 \cdots b_n$}{quien es $n$? la cantidad de variables? si es así esto no es consistente con lo que has estado diciendo, porque la cadena de las occurrencias de variables no necesariamente tienen n caracteres}, donde $b_i$ es el valor, $1$ o $0$, que
...
representación \comment{simbólica}{aquí mas bien sería una representación finita de tamaño polinomial con respecto a la fórmula. No te sirve una representación de cualquier tamaño, porque el vacío puede ser polinomial y la representación no, luego aplicarlo sobre eso, no sería polinomial} de la intersección, sobre la que estudiar
