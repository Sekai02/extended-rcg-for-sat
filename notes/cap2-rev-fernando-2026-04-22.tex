...
terminal $a$ que encabeza el argumento $aX$. \notaparaelautor{¿Y entonces qué pasa? Me quedé con la intriga \texttt{:-D}.}
...
subconjunto se utiliza en~\cite{aguilera2016srcg} para describir \queesesto{el
orden de las variables de una fórmula booleana} en una variante del SAT \notaparaelautor{¿en cuál? Aquí puedes poner algo como: en una variante del SAT que se denominó TAL COSA}.
...
complemento de una RCG es también una RCG~\comment{\cite{boullier1998proposal}}{Esto no lo pongas aquí, sino en algo del estilo: la demostración se puede ver en [REF].}.
\end{teorema}
...
Una vez presentadas \queesesto{las propiedades estructurales}, a continuación se
analiza la caracterización del formalismo en términos de complejidad
computacional, \comment{que constituye el resultado que motiva el uso de las RCG
en este trabajo}{Esto está \textit{misterioso}}.
...
tiempo polinomial\quitaesto{~citebertsch2001rcg, boullier2000range}.
\end{teorema}
...
que todo lenguaje RCG está en $PTIME$ \comment{se sigue}{esta expresión no la entiendo \texttt{:-(}.} de la existencia de un
...
revisan los antecedentes directos \comment{del trabajo}{¿de qué trabajo? ¿del tuyo o de los que estás referenciando?}.
...
$O(n^3)$ \comment{para ciertas subclases del SAT}{Parece que dices que operan en $n^{3}$ para ciertas subclases, y para otras en otra complejidad. Arréglalo para que quede claro a qué te refieres.}.
...
las fórmulas satisfacibles, \comment{y a partir de ella se derivan afirmaciones
sobre la expresividad del formalismo con respecto a la clase $NP$}{Esto está misterioso.}.
...
gramaticales y el SAT. \comment{El presente trabajo se inscribe en este programa
...
construcciones de RCG para variantes polinomiales del SAT.}{Bueno, esto lo vamos a dejar para más tarde para ver qué se hace al final \texttt{:-D}.} Estos
