...
Las gramáticas de concatenación de rango son el formalismo sobre el
que se desarrolla el resto del trabajo. El capítulo se organiza en
tres secciones. La sección~\ref{sec:rcg} introduce las gramáticas de
concatenación de rango, su presentación intuitiva, sus definiciones
formales y un ejemplo de reconocimiento. La sección~\ref{sec:propiedades-rcg}
expone las propiedades de clausura del formalismo y su caracterización
en términos de complejidad. La sección~\ref{sec:antecedentes} revisa
los antecedentes directos del trabajo: los estudios previos de la
...
porque, como se establece más adelante, los lenguajes que se reconocen
por una pRCG son exactamente aquellos cuyo problema de la palabra está
en $P$, y los resultados de los capítulos posteriores se apoyan en
dicha caracterización. Para abreviar, a lo largo del documento se adopta el
...
A continuación se introducen las nociones de predicado, argumento,
...
$B$ no reconoce este vector.

A las producciones de esta gramática se les denomina \textbf{cláusulas}.
El lado izquierdo de la producción siempre está formado por un único
predicado. El lado derecho puede contener cualquier cantidad de
...
Continuando con el ejemplo, se supone que los valores de $X$, $Y$ y
...
valores de las variables e instanciar los lados derechos se repite en
...
$A(x_1, \dots, x_k) \to \varepsilon$, donde $k$ es la aridad de $A$ y
cada $x_i$ es un argumento de $A$, esto es, una cadena de variables y
terminales.
...
El formalismo toma su nombre de la noción de rango, que identifica las
subcadenas asignadas a las variables.

\begin{definicion}
Un \textbf{rango} de una cadena es un par $(i, j)$ de posiciones, con
$i$ y $j$ enteros no negativos tales que $i \le j$, que delimita la
subcadena comprendida entre la posición $i$ y la posición $j$.
\end{definicion}

Por ejemplo, con las posiciones indexadas desde $0$, para la cadena
$abcd$ el rango $(1, 3)$ delimita la subcadena $bc$.
...
\begin{itemize}
  ...
  \item $P$ es un conjunto finito de \textbf{cláusulas} de la forma
    ...
    $n = 0$, el lado derecho es la cadena vacía y la cláusula se
    denomina terminal;
...
\begin{definicion}
Una \textbf{sustitución de rango} es un mecanismo que asigna a cada
variable un rango de la cadena que se reconoce, de acuerdo con la
estructura del argumento. La variable se reemplaza entonces por la
subcadena que ese rango delimita.
\end{definicion}
...
\begin{definicion}
\label{def:tamano-rcg}
El \textbf{tamaño} de una RCG $G = (N, T, V, P, S)$, que se denota $|G|$, es
\[
  |G| = |N| + |T| + |P|,
\]
la suma de las cantidades de predicados, símbolos terminales y cláusulas.
\end{definicion}
...
\begin{definicion}
\label{def:no-lineal}
Una RCG es \textbf{no lineal} si una misma variable aparece más de una
vez en el lado derecho de una cláusula.
\end{definicion}

Por ejemplo, la cláusula $A(X) \to B(X)\, C(X)$ es no lineal, pues la
variable $X$ aparece en dos argumentos del lado derecho.
...
contexto~\cite{hopcroft2006introduction}. Aun así, se reconoce por una RCG. En
particular, el lenguaje $L_{\mathrm{copy}}^3$ se reconoce por la
gramática $G_{\mathrm{copy}}^3$ del ejemplo anterior.

La próxima subsección analiza el problema de la palabra de las RCG y su
complejidad.
...
Un lenguaje se reconoce por una RCG en su variante positiva si y solo
si su problema de la palabra está en
$P$~\cite{bertsch2001rcg, boullier2000range}.
\end{teorema}

La dirección que afirma
que todo lenguaje que se reconoce por una RCG tiene su problema de la
palabra en $P$ se sigue de la existencia de un algoritmo de
reconocimiento polinomial para el formalismo, que se describe a
continuación.
...
además, la dirección inversa: todo lenguaje cuyo problema de la palabra
está en $P$ se reconoce por una RCG.
...
reconoce exactamente los lenguajes cuyo problema de la palabra está en
$P$, en la siguiente sección se revisan los antecedentes del trabajo.
...
verdaderas, bajo el supuesto de que cada ocurrencia de variable booleana se trata
por separado. Sobre ese lenguaje se impone después que las ocurrencias
de una misma variable booleana tomen el mismo valor, mediante una gramática. La
...
$0$ el falso. Cada ocurrencia de variable booleana de la fórmula aporta uno de
...
\end{tabular}
\caption[Autómata booleano de la negación]{La negación intercambia el papel de $V$ y $F$: en $\neg A$ el
...
\begin{tikzpicture}[->, >=stealth, shorten >=1pt, auto, semithick]
  ...
  \draw (B2) -- (F);
\end{tikzpicture}
\caption[Autómata booleano de la disyunción]{Disyunción $A_1 \vee A_2$. Al alcanzar $V_1$, un camino de
...
La conjunción $A_1 \wedge A_2$ se construye de forma análoga a la
disyunción, con los papeles de $V$ y $F$ intercambiados. Si en el
autómata de $A_1$
...
del autómata de $A_2$. La figura~\ref{fig:automata-conjuncion} lo
muestra.
...
\begin{tikzpicture}[->, >=stealth, shorten >=1pt, auto, semithick]
  ...
  \draw (B2) -- (F);
\end{tikzpicture}
\caption[Autómata booleano de la conjunción]{Conjunción $A_1 \wedge A_2$. Al alcanzar $F_1$, un camino de
...
\end{tabular}
\caption[Autómatas de las subfórmulas de $A$]{Autómatas de $x_1$ y de $x_2 \vee x_1$, las dos subfórmulas de
$A$.}
...
\begin{tikzpicture}[->, >=stealth, shorten >=1pt, auto, semithick]
  ...
  \path
    ...
    (q5) edge node {$0,1$} (F);
\end{tikzpicture}
\caption[Autómata booleano de $A = x_1 \wedge (x_2 \vee x_1)$]{Autómata booleano de la fórmula $A = x_1 \wedge (x_2 \vee
...
El autómata booleano de una fórmula con $n$ ocurrencias de variable booleana
tiene $O(n^2)$ estados~\cite{fernandezarias2007}. En él todas las
ocurrencias se tratan por separado, incluso las de una misma variable booleana, y
por eso hace falta un segundo mecanismo que imponga la consistencia
entre las ocurrencias.

La próxima subsección presenta la gramática que impone esa consistencia
y la clase de fórmulas que cada trabajo resuelve con ella.

\subsection{La consistencia entre ocurrencias y la clase de fórmulas}
\label{subsec:consistencia-clase}

Una cadena de ceros y unos, con un símbolo por
ocurrencia, es \textbf{consistente} si en las posiciones de cada variable booleana
...
equivalente a un autómata de pila~\cite{fernandezarias2007}. En la pila
del autómata se guarda el valor de una variable booleana en su primera
ocurrencia y se compara en las siguientes. Como en una pila se accede a los valores en orden inverso al
...
consecutivas de una variable booleana aparece una ocurrencia de otra, todas las
...
solo fragmento de la cadena, una sRCG admite varios a la vez, por lo que
...
\end{tabular}
\caption[Órdenes de ocurrencias: anidado y con cruce]{Arcos que unen las ocurrencias de cada variable booleana. En (a) los
...
separados de la cláusula, y la cantidad de argumentos, esto es, la
aridad, crece con el cruce. Con la no linealidad esa igualdad se expresa de forma
directa: una misma variable aparece más de una vez en el lado derecho
...
trabajo de la Facultad en esa dirección~\cite{artechemunoz2026}.}
