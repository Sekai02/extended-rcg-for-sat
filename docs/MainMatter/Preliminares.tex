...
usados en el resto del trabajo. En la
sección~\ref{sec:teoria-lenguajes} se introducen las nociones
básicas de la teoría de lenguajes. Después, en la
sección~\ref{sec:automatas}, se presentan los autómatas finitos. Por
último, en las secciones~\ref{sec:complejidad} y~\ref{sec:sat}, se
introduce la complejidad computacional y se presenta el problema de la
...
de Chomsky, que clasifica los lenguajes formales según las gramáticas que
los generan.
...
$\Sigma = \{0, 1\}$ está formado por los símbolos $0$ y $1$; algunas
cadenas sobre este alfabeto son $11$ y $101$, y un lenguaje sobre él es el
conjunto de cadenas formadas por $0$ y $1$ que terminan en $1$.

La \textbf{longitud} de una cadena $w$, denotada $|w|$, es la cantidad de
símbolos que la componen. El símbolo $\varepsilon$ representa la
\textbf{cadena vacía}, que no tiene símbolos, de modo que
$|\varepsilon| = 0$.

Dadas dos cadenas $w_1$ y $w_2$, la \textbf{concatenación} $w_1 w_2$
es la cadena formada por los símbolos de $w_1$ seguidos de los símbolos
...
Por ejemplo, para el lenguaje $L$ de las cadenas binarias que terminan
en $0$, el problema de la palabra consiste en decidir, dada una cadena
$w$, si $w \in L$; así, $1100 \in L$ y $1101 \notin L$.
...
decidir si tal lenguaje tiene algún elemento.
...
A continuación se presentan las gramáticas, un mecanismo
...
\begin{itemize}
  ...
  \item $P$ es un conjunto finito de \textbf{reglas de producción} de la
    ...
    \[
      \alpha \to \beta,
      \quad \text{con } \alpha \in (N \cup \Sigma)^* \setminus \Sigma^*
...
$\alpha \to \beta$, donde $\alpha \in (N \cup \Sigma)^* \setminus \Sigma^*$
y $\beta \in (N \cup \Sigma)^*$. Todo lenguaje generado por una gramática
...
De los modelos asociados a las clases de la jerarquía, en este trabajo
solo se emplea el autómata finito, que reconoce los lenguajes
regulares~\cite{hopcroft2006introduction}.
...
de decisión} es un problema cuya respuesta para cada
instancia\footnote{Una instancia es una entrada del problema.} es sí
o no.
...
candidata\footnote{Este término también se conoce en la literatura
como certificado.}, verifica en tiempo polinomial si la solución
candidata es correcta~\cite{hopcroft2006introduction}.
\end{definicion}
...
constituye uno de los problemas abiertos de la teoría de la
...
\begin{itemize}
  \item \textbf{Variables booleanas:} una variable booleana es una
    variable que puede tomar uno de dos valores: verdadero o
    falso.
  \item \textbf{Literales:} un literal es una variable booleana o su
    ...
    negación de $x$. Un literal toma los valores verdadero o
    falso según la asignación de valores a las variables booleanas.
  ...
  \item \textbf{Fórmulas en forma normal conjuntiva:} una fórmula
    ...
    \[
      ...
      (x \vee \neg z)
    \]
    es una fórmula en CNF con tres cláusulas y tres variables booleanas.
\end{itemize}

Una \textbf{interpretación} de una fórmula booleana es una asignación
de valores verdadero o falso a cada una de sus
variables booleanas.

Una fórmula booleana es \textbf{satisfacible} si es verdadera bajo
alguna de sus interpretaciones.

\begin{definicion}
El problema de la \textbf{satisfacibilidad booleana}, o SAT, consiste
en determinar si una fórmula booleana es satisfacible.
\end{definicion}

Toda fórmula booleana se puede transformar, en tiempo polinomial, en
una fórmula en CNF que es satisfacible si y solo si la fórmula
original lo es~\cite{hopcroft2006introduction}. Por esta razón, en el
resto del trabajo se asume sin pérdida de generalidad que toda
fórmula booleana está en CNF.
...
$NP$ porque, dada una asignación de valores a las variables booleanas de la
...
1971, y marcó el inicio de la teoría de la
$NP$-Completitud~\cite{cook1971}.
...
del problema $n$-SAT se puede reducir a una fórmula del problema
3-SAT que es satisfacible si y solo si la fórmula original lo es, por
lo que SAT y 3-SAT son equivalentes en términos de complejidad
computacional~\cite{hopcroft2006introduction}.
...
en que se apoya ese enfoque: la teoría de lenguajes, los autómatas
finitos, la complejidad computacional y el problema SAT. En el próximo capítulo
